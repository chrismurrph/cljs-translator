(ns restaurant.with-customer.mutations.till
  (:require
   ...
   [restaurant.with-customer.mutations.calc :as wc-muts-calc]
...
(defn new-demo-till [st]
  (let [{new-id :till/id :as new-till} (mk-till (wc-ui-till/create-demo-till r-domain/denominations 10))]
    (->> st
...
(defn check-out [st {:bill/keys [id given-back-m] :as current-bill} current-till-id]
  ...
  (->> st
    ...
    #_(r-muts/set-session-event :session-event.type/check-out current-bill)
    (#(r-muts/inject-many % [:session-event-data :session-event-type]
        [current-bill
         :session-event.type/check-out]))))

(comment
  (let [{:keys [current-till-id] :as st} (new-demo-till {})
    ...
    (case calc-state
      ...
      :billing-out (let [increase-denomination-f (delta-denomination-rf inc)
                     ...
                     (if (and (-> config :auto-give-change :b?) (pos? now-amt))
                       (wc-muts-calc/give-change st0 current-bill-id now-amt till)
                       st0)))))
