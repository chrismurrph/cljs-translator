  ...
  (when (> n 1)
    [:div
     {:style {:pointer-events "none"
              ...
              :z-index z-index}}
     [:svg
      (for [[opacity rect-path] (map vector utl/opacities (utl/rect->path-dims dims n))]
        ^{:key (str dims)} [:path
                            {:opacity opacity
                             :d rect-path}])]]))
...
(defn denomination-event [str-kind denomination disabled? amount current-bill-id current-till calc-state selected? config]
  (when-not disabled?
    (let [negate? (and (= :giving-change calc-state) selected? (zero? amount))]
      ...
      (dispatch [::wc-till-events/receive-note current-bill-id current-till calc-state denomination amount config negate?]))))
   ...
   [:div
    ...
    (for [bill-denomination note-values]
      (let [bill-denomination-count (or (get bill-denominations bill-denomination) 0)
        ...
        ^{:key (str face-value)} [:div
                                  {:style {:position "relative"}}
                                  [:button {:on-click event-f
